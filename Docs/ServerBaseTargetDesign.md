...
- **token 契约无过期字段（契约债，需与客户端共同排期）**：服务端内部 TTL 可先行，
  但客户端无法预判过期、只能被动 401 后重登；把 `expiresAt` 加进 Auth 响应属于契约演进
  （ADR-S001），服务端单方面无法闭环。
...
- 帧同步/房间语义是否进入 ServerBase 主干：默认不进，若做则以扩展包形态
  （对齐客户端扩展包惯例）。
- **埋点/崩溃端点无鉴权（契约债，需与客户端共同排期）**：客户端未实现签名头，
  §6.3 的限流+体积限制是首期唯一防线；加轻量签名头属于契约演进，服务端单方面无法闭环。
