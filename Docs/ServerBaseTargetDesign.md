...
- 请求体：`{ "mode":"guest|account", "account", "password", "sessionToken", "deviceId" }`；
  响应体：`{ "success":bool, "userId", "sessionToken", "errorCode", "errorMessage" }`。
...
| 七 | Crash ingest（JSON Lines 解析、原文落盘） | 客户端 LocalFileCrashBackend 积压上报打通（低优先） |
...
1. **token 生命周期**：现行契约 `sessionToken` 无过期/刷新字段——首期在服务端内部定义
   TTL + 滑动续期（每次重绑/心跳鉴权成功刷新），token 采用带签名的不透明令牌，
   服务端可单方失效。向契约加 `expiresAt`/refresh 属于契约变更，留待 ADR-S001 二期。
...
- **token 契约无过期字段**：服务端内部 TTL 可先行，但客户端无法预判过期、只能被动
  401 后重登；把 `expiresAt` 加进 Auth 响应属于契约演进，需与客户端排期（ADR-S001）。
...
- 帧同步/房间语义是否进入 ServerBase 主干：默认不进，若做则以扩展包形态
  （对齐客户端扩展包惯例）。
- 埋点/崩溃端点无鉴权是既定现状（客户端未实现签名头）；§6.3 的限流+体积限制是
  首期唯一防线，是否加轻量签名头属于契约演进，与客户端一同排期。

## 附录 A：ADR 条目雏形（服务端仓库建立后正式编号入库）

| 编号 | 决策 | 理由与代价 |
| --- | --- | --- |
| ADR-S001 | Auth 沿用客户端参考 JSON 契约与单端点形态；token 生命周期首期只在服务端内部（TTL+滑动续期），不改契约 | 目标 1"零客户端改动"优先；代价是客户端无法预判过期，二期加 `expiresAt` 时再过冻结点 |
