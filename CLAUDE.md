...
注释的参照系只能是**当前代码**，不能是它的历史版本。

```csharp
// 反例：读者需要想象一个他没见过的过去才能读懂
// 与旧实现的关键区别：安装成功后不再立即删除 .bak
// 历史缺陷：旧实现用单个 bool 同时表示"没有更新 / 下载失败 / 校验失败"

// 正例：同样的信息量，参照系换成当前
// .bak 保留到统一启动确认点，中途失败可整轮回滚
// 分成四种结果，避免"没有更新"与"安装失败"被调用方混为一谈
```

判据：把注释给一个从没见过旧版本的人读，若他必须先知道"以前是怎样"才能理解，就得改写。

变更叙事归 commit message / ADR / CHANGELOG——那里已经记了，代码里再留一份副本
只会两处不同步。同理，注释里不写本次会话/评审的上下文（"按上一轮讨论"、"改为…"、
"本轮清理"），也不写会过期的快照数字。

例外：**当前仍存在**的兼容对象可以点名，因为它描述的是现状而非历史——如
"兼容旧 Prefab 的 _path 序列化数据"、"清理上一版遗留的单文件产物"、
"拒绝旧 hmac256 格式存档"。这些旧东西此刻真的在磁盘上。

### 4. 注释与实现须同步

注释与实现不符视为 bug。删除逻辑时一并删除其注释。
改动行为语义时须复查该类型/方法的注释是否随之失效。
