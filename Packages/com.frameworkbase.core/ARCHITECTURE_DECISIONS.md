...
8. **账号级/帧级生命周期作为模块契约的可选阶段，由宿主有序驱动**（本条修订自初稿的“账号走事件”）。
   实施红点时发现事件模型不成立：红点已看版本必须在业务入口**之前有序 await 加载**（否则“先亮后灭”，
   这是既定设计目标），且需**逐帧** `FlushPending` 结算——fire-and-forget 的 event 既无法表达“等我加载
   完再进业务”的有序 await，也无法逐帧驱动。故 `IFrameworkModule` 增加三个**可选**钩子：
   `OnAccountEnterAsync`（宿主在业务入口前有序 await）、`OnAccountExit`（身份清除前同步收尾）、
   `OnLateUpdate`（每帧结算）；`FrameworkModuleBase` 提供空实现，引导模块不覆盖、零影响；`RedDotModule`
   在这些钩子里做账号会话 Begin/End 与帧末结算。`GameEntry` 只驱动宿主、不认识具体模块，依赖方向不破。
   引导那类纯事件驱动、一次性启动的模块用不到这些钩子——它们是为红点这类有状态、账号感知的模块准备的。
...
### ADR-008 实施记录（2026-07-22）

**状态**：已实施（步骤 1–5 全部落地，逐步过 CI：编译 + EditMode + asmdef 门禁 R1–R6 + 资源门禁）。

**运行时三层落地**：`Framework`(L1) 引入模块宿主 `FrameworkModuleHost` + 契约 `IFrameworkModule`（两阶段
`RegisterCapabilities`→冻结编排→`StartAsync`，外加可选 `OnAccountEnterAsync`/`OnAccountExit`/`OnLateUpdate`）。
引导收敛为 `GuideModule`、红点收敛为 `RedDotModule`（均迁入 `Framework.Modules` asmdef，命名空间不变），
访问点改为 `Guides.Runner` / `RedDots.Service`。`GameEntry` 移除 `Guides/RedDots/GuidePresentation` 字段，
只保留能力入口 + 宿主驱动，不再认识任何具体模块。R6 门禁把"L1 不引用 `Framework.Modules*`"焊进 CI。

**Editor 侧的形态抉择（回答"中间层复杂起来 Editor 会不会有问题"）**：`ConfigPipeline`（导出聚合器）与
`CiGate`（门禁聚合器）天然要"认识各模块的编译器/校验器"，这是经典的聚合器/插件问题，有三种形态：

- **形态 A**：模块 Editor 工具全塞一个 asmdef、聚合器同程序集内直接调。无环但聚合器每加模块要改、分层不纯。
- **形态 B（陷阱）**：模块 Editor 各自 asmdef + 聚合器静态引用 → `Framework.Editor` ↔ 模块 Editor **成环**。
- **形态 C（本次采用）**：**依赖倒置 + 发现**。契约 `IConfigArtifactExporter`/`IBuildValidator` 放
  `Framework.Editor`；模块 Editor 工具实现契约并迁入 `Framework.Modules.Editor`（单向依赖 `Framework.Editor`
  的契约/`ExcelReader`）；聚合器用 `UnityEditor.TypeCache` 发现全部实现遍历执行，**不静态引用任何具体编译器**。
  依赖方向 `Framework.Modules.Editor → Framework.Editor` 单向、无环；`Framework.Editor` 不再引用
  `Framework.Modules`。

**为何直接上形态 C 而非留触发条件**：中间层业务若持续变复杂，形态 A 的"聚合器扇出 + 一旦独立 asmdef 就成环"
迟早爆发；形态 C 用 `TypeCache`（编译期建索引、无运行时反射开销）一次性把聚合器与模块解耦——**新增带配置/
门禁的模块，只要实现契约即被自动纳入导出与门禁，聚合器零改动**。这与运行时侧"宿主 + `IFrameworkModule` 注册
驱动"完全对称：两侧都是"契约 + 发现/注册"，心智负担最低。`TypeCache` 发现是 Unity 及大厂 Editor 扩展做插件
发现的标准手段（等价物：MSBuild task 发现、Roslyn source generator、DI 程序集扫描注册）。

**布局最终态**：
```
Framework.Kernel / Framework.Foundation / Framework(L1 能力：UI(含 Targets)/宿主/通用遮罩件/编排原语)
Framework.Modules(L2：Guide/ + RedDot/)          Framework.Modules.Editor(L2 Editor：Guide/ + RedDot/ 编译器与窗口)
Framework.Editor(L1 Editor：ExcelTool/CiGate/ConfigPipeline/契约/UIWindow 编译器)
L3 HotUpdate：OrchestrationBootstrap 冻结全局编排；RuntimeCatalogBootstrap 经 Modules.Use 登记各模块
```

### ADR-008 修订：架构复审后的六项收口（2026-07-22）

首轮落地后做了一次分层复审，暴露出"边界立住了但归属没划干净"的若干处，逐项收口：

**第 9 点 · 模块宿主的异常策略分两段**。原实现把 Phase 1/2 的异常与运行期回调一样吞掉，后果有二：
模块半装配后继续接收账号/帧回调；且真正根因会被随后的编排冻结伪装成"TypeId 未注册 Executor"这类
二级错误。现定为——**启动阶段 fail-fast**（标记模块 `Faulted` 后原样抛出，让登录失败在根因处），
**运行阶段隔离**（低内存/账号/帧回调单模块异常不影响其它模块）；`Faulted` 模块不再接收任何后续回调，
惟 `Dispose` 例外（可能已持有待释放资源）。

**第 10 点 · 全局编排装配独立于任何模块**。Rule/Trigger/Action 三个目录是全框架共享基础设施，原先
寄生在 `GuideBootstrap` 并以 `GameEntry.OnFreezeOrchestration = ...` 单槽赋值持有钩子——第二个需要
Payload 工厂的模块出现即静默覆盖。现拆出 L3 的 `OrchestrationBootstrap` 独占钩子，对外开放
`Register{Rule,Trigger,Action}PayloadFactory`；工厂以"构建器"形式登记、冻结时才求值，索引因而建在
配置库确定就绪的时刻。这补上了运行时侧长期缺失的、与 Editor 侧形态 C 对称的那一半：**模块既能自注册
Executor，也能自带 Payload 工厂，新增模块只改自己的 Bootstrap**。

**第 11 点 · 编排 TypeId 号段规约**。R6 门禁拦得住程序集引用方向，拦不住"L1 号段表里出现业务概念"
（`GuideFocusTarget` 曾定义在 L1 的 `BuiltinOrchestrationTypeIds` 里）。现与协议主号、广播枚举同构地
划段：Rule 千位 1 / Trigger 千位 2 / Action 千位 3，每类内 **x001-x099 L1 框架段**（只放 UI/时间等中立
能力）、**x100-x199 L2 模块段**（各模块在自己的程序集内定义）、**x200 起 L3 项目段**。引导的 3004/3005
是规约建立前的历史占位，归属已迁至 `GuideOrchestrationTypeIds`，数值待下次重导 `config.db` 时迁段。

**第 12 点 · 访问点发布是生命周期动作，不是构造副作用**。`RedDotModule` 原在构造函数里写
`RedDots.Service`，new 第二个实例即静默顶掉当前实例，测试之间也会串状态。发布改到 Phase 1
`RegisterCapabilities`——仍早于业务入口，UI 徽标可在目录初始化前先订阅。各模块 `Dispose` 一律以
`ReferenceEquals` 守卫后再清访问点。

**第 13 点 · Target 目录归 UIManager**。`UITargetRegistry` 是纯 UI 概念、生命周期与 UI 一致，挂在
`GameEntry` 上只是让静态门面继续膨胀（ADR-003 "不新增门面"的另一面是也别往既有门面上堆）。改为
`UIManager.Targets`。

**第 14 点 · 测试程序集按层拆分**。核心测试集原先直接引用 `Framework.Modules*`，L1/L2 测试混在一起；
模块只会越来越多，一个模块的测试编译失败会拖垮整个核心测试集。引导/红点测试迁入
`Framework.Modules.Tests.EditMode`，并把"核心测试集不得引用中间层"一并加进 R6 门禁。
...
## ADR-011：窗口焦点只在移动端代表进入后台（2026-08-03）

**状态**：已实施。`NetworkHostKind` / `NetworkHostProfile`（`Network/NetworkLifecycle.cs`）给出宿主形态与
规则，`NetworkManager.OnApplicationFocus` 按规则决定是否 `EnterBackground`。规则纯逻辑可测。

**背景**：网络层的后台态是一个很重的状态——`NetworkManager.OnUpdate` 在后台态下于入站消息泵之前整条
返回，于是入站派发、在途请求超时、心跳、重连退避同时停摆。这个代价只有在进程真的被系统挂起时才划算。

移动端进后台由 `OnApplicationPause` 权威通知，`OnApplicationFocus` 是部分平台漏发 Pause 时的兜底，
两者都指向同一件事。但焦点信号在其余形态里表达的完全不是这件事：

- 桌面窗口（独立包与编辑器）：玩家 alt-tab、点开浏览器都会失焦，进程一直在跑。把它当进后台，
  表现为切出去再切回来之前连接被冻住——在途请求连超时都不会触发，服务端按空闲回收连接。
- 无头进程（batchmode 的 CI、自动化联调）：Unity 恒报无焦点，网络层从启动起就是后台态，
  症状是"连得上、发得出、永远收不到"。

**决策**：焦点丢失是否等于进入后台，由宿主形态决定，只有 `Mobile` 为真。得焦一侧不加形态判断——
本形态虽不认失焦，Pause 仍可能已把状态置为后台，恢复判定必须照常走。

**为什么不改为"后台态只停心跳、保留入站泵"**：那会把一个被系统挂起的进程假装成活着的，
后台态的全部意义正是承认"这段时间我们什么都做不了"，恢复时才需要按后台时长决定探活还是废弃旧连接。
问题不在后台态的语义，而在谁有资格宣告它。

**为什么用编译期宏而非 `Application.isMobilePlatform` 判定形态**：后者在编辑器下的取值随激活构建目标
漂移，会让"编辑器里点开别的窗口是否冻网络"取决于当前切到了哪个平台——这种依赖不该存在。

**放弃了什么**：不再让桌面端借焦点做省电/省流量的连接挂起。真需要时应由业务显式调用，
而不是从一个平台语义不一致的信号里推断。
