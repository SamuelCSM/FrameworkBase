...
namespace Framework
{
    ...
    /// Catalog 更新流程的终态分类。
    /// <para>
    /// 每一种终态必须显式分开，<b>禁止退化成单个数值</b>：用"更新数量"表示全部结果时，
    /// 返回 0 会同时意味着"没有更新 / 检查失败 / 检查抛异常 / 更新失败 / 更新抛异常"五种语义，
    /// 调用方无从区分"无需更新"与"更新失败"，失败链路最终会被误提交 ResourceVersion。
    /// </para>
    /// </summary>
    ...
    /// <summary>
    /// 下载尺寸查询结果状态。
    /// 把"无需下载（0 字节）"和"查询失败"显式分成两种结果：二者一旦都表示为 0，
    /// "下载尺寸查询失败"会被误判为"所有 bundle 已是最新"。
    /// </summary>
