...
| 层级 | 适用数据 | 实现 |
|------|---------|------|
| **SaveManager.SaveAsync** | 游戏存档、玩家进度、背包等结构化数据 | JSON + AES-128-CBC 加密 + HMAC-SHA256 完整性校验 |
...
> ⚠️ **设备密钥失效边界（务必知悉）**：默认主密钥种子 = `SystemInfo.deviceUniqueIdentifier`。
> 该值在部分场景会**变化**——Android 应用重装或隐私重置、iOS 同厂商 App 全部卸载后重装等。
> 一旦设备标识改变，旧存档与用同一设备密钥加密的 `EncryptedPrefs` 将**全部无法解密**，
> 读档 fallback 到默认值，表现为"本地进度丢失"。这是设备绑定密钥的固有代价，非缺陷。
> **重要进度必须托底到云存档**（见「跨设备 / 上云存档」注入服务端下发密钥），
> 不能只依赖本地存档。派生用的是单遍 `SHA-256`（对高熵设备种子够用）；若改注入**低熵/口令**
> 类密钥来源，务必换成 PBKDF2/Argon2 + 每档随机 salt，不要沿用当前单遍派生。
...
AES 加密后文件是二进制密文，玩家无法直接编辑。HMAC-SHA256 完整性校验确保即使文件被修改也会被检测到并拒绝加载（回退到备份档）。
