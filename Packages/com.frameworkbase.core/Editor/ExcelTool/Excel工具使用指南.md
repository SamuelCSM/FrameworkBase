...
第0行：字段注释（中文说明）
第1行：字段名称（C#属性名）
第2行：字段类型（int/long/float/string/bool/枚举名 等）
第3行及以后：数据行
```

> ⚠️ **类型行不可省略**：第2行必须是类型定义，数据从第3行开始（对应 `ExcelReader.ExcelFormat` 默认
> `TypeRowIndex=2` / `DataStartRowIndex=3`）。漏掉类型行会让第一条数据被当类型读走，生成出
> `public 1 Id` 这样的非法代码。
...
- **第2行（类型行）**：字段的C#类型（int/long/float/double/string/bool/枚举名 等），生成属性与SQLite列类型据此确定
- **第3行及以后（数据行）**：实际的配置数据
