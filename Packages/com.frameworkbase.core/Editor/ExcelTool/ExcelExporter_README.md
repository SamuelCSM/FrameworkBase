...
配表默认按客户端和服务端双端导出，不需要逐表配置。默认规则在
`Packages/com.frameworkbase.core/Editor/ExcelTool/ConfigExportRules.asset` 中维护；项目可在
`Assets/Editor/ExcelTool/ConfigExportRules.asset` 放置同类型资产覆盖包默认值：

- `Both`：客户端 SQLite 与服务端 TSV 都导出，默认值
- `ClientOnly`：只导出客户端，不生成服务端 `.txt` 与服务端配置类
- `ServerOnly`：只导出服务端 TSV/配置类
- `ToolOnly`：仅作为专用工具输入，不进入通用 SQLite、TSV 和配置类生成流程

规则还可设置表形态：`Keyed` 使用首列主键与 `ConfigBase<TKey,TValue>`；`List` 不创建主键并生成
`ConfigListBase<TValue>`，适合关系表和多对多表。纵向单例配置继续由 `_general` 工作表后缀识别。

当前默认规则中，`language`、`loading_tips`、`ui_wnd_res` 和红点五表为 `ClientOnly`；
`red_dot_edge_ref` 额外使用 `List` 形态，因此允许多个关系拥有相同 `ParentId`。
