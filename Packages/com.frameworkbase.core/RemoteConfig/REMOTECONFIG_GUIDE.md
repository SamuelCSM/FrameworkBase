...
三个字段都可以缺省（缺省即该维度不构成限制），但**出现就必须合法**：

| 非法写法 | 结果 |
|---|---|
| `"enabled": "yes"` / `1` | 整份远端值作废 + 告警（只认 `true`/`false` 与 `"true"`/`"false"` 文本） |
| `"rollout": 150` / `-1` / `"abc"` | 整份远端值作废 + 告警（须为 0～100 的整数） |
| `"min_version": "1.2.x"` / 非字符串 | 整份远端值作废 + 告警（须为可比较的数字点分版本） |

作废后判定退回本地基线：代码默认值 → 调用方兜底参数。这是刻意的 fail-closed——
宽容地跳过非法字段等于让"配置写错"变成"限制消失"，一个 typo 就能把灰度中的功能全量放出去。
告警对同一键每份配置只出一次，避免每帧调用刷屏。
