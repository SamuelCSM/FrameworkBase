...
internal static class Program
{
    ...
    /// <summary>依命名约定生成路由伴生 partial（GetMainId/GetSubId + IRequest/IResponse）。</summary>
    private static void GenerateRouting(List<ProtoMessage> messages, string outDir, string routingNs)
    {
        // 建索引，供 GC2GS 请求配对同主/子号的 GS2GC 响应。
        ...
        foreach (IGrouping<string, ProtoMessage> group in messages.GroupBy(m => m.Namespace))
        {
            ...
            sb.AppendLine("}");
            File.WriteAllText(Path.Combine(outDir, "ProtoRouting.g.cs"), sb.ToString(), new UTF8Encoding(false));
        }
    }
}
...
/// <summary>解析出的一条协议消息。</summary>
internal sealed record ProtoMessage(string Namespace, string ClassName, string Direction, byte MainId, byte SubId, bool HasResultCode);
